...
class _ProposalFormState extends State<ProposalForm> {
  ...
  final TextEditingController _budgetController = TextEditingController();
  bool _useEscrow = false; // Variable to track escrow selection
  File? _coverLetterFile; // Variable to store selected file
  ...
  Future<void> _submitProposal() async {
    ...
    if (name.isNotEmpty && budget != null) {
      ...
      request.fields['use_escrow'] = _useEscrow.toString();

      if (_coverLetterFile != null) {
        request.files.add(
          await http.MultipartFile.fromPath('cover_letter', _coverLetterFile!.path),
        );
      }
      ...
      try {
        ...
        if (response.statusCode == 201) {
          ...
          // Handle error response
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Error: Unable to submit proposal.')),
          );
        }
      } catch (e) {
        // Handle network or other errors
          ...
          Row(
            children: [
              ElevatedButton(
                ...
                child: Text('Attach Cover Letter'),
              ),
              SizedBox(width: 8.0),
              Expanded(
                child: Row(
                  children: [
                    Expanded(
                      child: Text(
                        _coverLetterFile != null
                            ? 'File: ${_coverLetterFile!.path.split('/').last}'
                            : 'No file selected',
                        style: TextStyle(fontSize: 12.0),
                    ...
                    if (_coverLetterFile != null)
                      IconButton(
                        onPressed: () {
                          setState(() {
                            _coverLetterFile = null; // Clear the selected file
                          });
                        },
